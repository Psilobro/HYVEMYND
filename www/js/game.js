...
function setupZoomControls(app) {
    ...
    // Add visual debug overlay for touch events
    ...
    debugDiv.style.cssText = `
        ...
        max-width: 400px;
    `;
    debugDiv.textContent = 'Touch Debug: Ready...';
    document.body.appendChild(debugDiv);
    
    // Override touch events with better debugging
    app.view.removeEventListener('touchstart', arguments.callee.touchStartHandler);
    app.view.removeEventListener('touchmove', arguments.callee.touchMoveHandler);
    app.view.removeEventListener('touchend', arguments.callee.touchEndHandler);
    
    let touchDebugData = { active: false, count: 0, distance: 0, zoom: 1.0 };
    
    const newTouchStart = (e) => {
        e.preventDefault();
        e.stopPropagation();
        touchDebugData.count = e.touches.length;
        touchDebugData.active = true;
        
        if (e.touches.length === 2) {
            isZooming = true;
            const touch1 = e.touches[0];
            const touch2 = e.touches[1];
            touchDebugData.distance = Math.sqrt(
                Math.pow(touch2.clientX - touch1.clientX, 2) +
                Math.pow(touch2.clientY - touch1.clientY, 2)
            );
            lastTouchDistance = touchDebugData.distance;
            debugDiv.textContent = `PINCH START: ${touchDebugData.count} touches, distance: ${touchDebugData.distance.toFixed(1)}px`;
        } else {
            debugDiv.textContent = `TOUCH START: ${touchDebugData.count} touch(es)`;
        }
    };
    
    const newTouchMove = (e) => {
        e.preventDefault();
        e.stopPropagation();
        ...
        if (e.touches.length === 2 && isZooming) {
            ...
            if (lastTouchDistance > 0) {
                const zoomFactor = currentDistance / lastTouchDistance;
                touchDebugData.zoom *= zoomFactor;
                
                debugDiv.textContent = `PINCHING: dist=${currentDistance.toFixed(1)} factor=${zoomFactor.toFixed(3)} zoom=${touchDebugData.zoom.toFixed(2)}`;
                
                const rect = app.view.getBoundingClientRect();
                const centerX = ((touch1.clientX + touch2.clientX) / 2) - rect.left;
                const centerY = ((touch1.clientY + touch2.clientY) / 2) - rect.top;
                
                console.log('DEBUG: Calling zoomAt with factor:', zoomFactor);
                zoomAt(centerX, centerY, zoomFactor);
                lastTouchDistance = currentDistance;
            }
        }
    ...
    const newTouchEnd = (e) => {
        ...
        if (e.touches.length < 2) {
            isZooming = false;
            lastTouchDistance = 0;
            debugDiv.textContent = `PINCH END: ${touchDebugData.count} remaining touches`;
        }
        if (e.touches.length === 0) {
            touchDebugData.active = false;
            debugDiv.textContent = 'Touch Debug: Ready for next gesture...';
            setTimeout(() => {
                if (!touchDebugData.active) {
                    debugDiv.textContent = 'Touch Debug: Listening...';
                }
            }, 2000);
        }
    };
    
    app.view.addEventListener('touchstart', newTouchStart, { passive: false });
    app.view.addEventListener('touchmove', newTouchMove, { passive: false });
    app.view.addEventListener('touchend', newTouchEnd, { passive: false });
    
    // Also try body-level events as fallback
    document.body.addEventListener('touchstart', (e) => {
        if (e.target === app.view) {
            debugDiv.textContent += ' [BODY CAUGHT]';
        }
    }, { passive: false });
}
